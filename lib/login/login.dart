...
class _LoginState extends State<Login> {
                        ...
                        Expanded(
                            ...
                            child: Center(
                              child: TextFormField(
                                ...
                                decoration:
                                    InputDecoration(hintText: "Contrasena"),
                                obscureText: true,
                                validator: (value) {
                                  if (value.isEmpty) {
                                    return 'Please enter some text';
                    ...
                    listener: (context, state) {
                      if (state is LoginCubitApiError) {
                        _notification(context,
                            'Error de inicio de sesion!. ' + state.error);
                        return;
                      }

                      if (state is LoginCubitRequestError) {
                        _notification(
                            context, 'Error de conexion!. Intentole mas tarde');
                        return;
                      }
  ...
  void _success(BuildContext context) async {
    ...
    await context
        ...
        .setIdCliente(context.bloc<LoginCubit>().res['id']);
    print("id cliente >>> " + context.bloc<ComprarMembresiasCubit>().idCliente);
    //pide las clases/solicitudes pasadas
    // await context.bloc<sol>()

    await context
        .bloc<SolicitudesPasadasCubit>()
        .getSolicitudesPasadas(context.bloc<LoginCubit>().res['id']);
    if (!rpta) {
      Navigator.push(
        context,
